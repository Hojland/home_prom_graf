...
class MockSMBus:
    ...
    def write_i2c_block_data(self, i2c_address, register, values):
        ...
        microbit.cmd(f"i2c.write({i2c_address}, {bytes(values)})")
...
class Device(object):
    ...
    def _i2c_read(self, register, bit_width):
        value = 0
        length = bit_width // self._bit_width
        for x in self._i2c.read_i2c_block_data(self._i2c_address, register, length):
            value <<= 8
            value |= x
        return value
